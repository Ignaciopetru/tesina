...
\begin{figure}[h!]
    \centering
    % Primer subentorno minipage (para Arduino)
    \begin{minipage}{0.4\linewidth}
        ...
        \label{arduinoUNO}
    \end{minipage}\hfill
    % Segundo subentorno minipage (para Raspberry)
    \begin{minipage}{0.4\linewidth}
        \centering
        ...
        \label{raspberry}
    \end{minipage}
    % Pie de figura para todo el conjunto (opcional)
...
\begin{itemize}
    \item Leer la temperatura de un sensor cuando hay una medición disponible.
...
Un sistema embebido de control robótico es una unidad de procesamiento diseñada para gestionar el funcionamiento de un robot, integrando sensores, actuadores y algoritmos de control. Por lo general estos sistemas están diseñados para operar en tiempo real y ejecutar tareas específicas llevando a cabo comportamientos básicos del robot relacionados al movimiento o adquisición de información. Por ejemplo, en \cite{paperPomponio} el sistema se encarga de ejecutar (o de responder a) las órdenes de una PC, las cuales establecen referencias para la velocidad y la dirección del robot. Para concretar las acciones se necesita llevar un control en tiempo real de los componentes físicos y para hacerlo se hace uso de todo el hardware disponible (sensores, actuadores, sistemas de comunicación, etc.).

Existen dos enfoques básicos para llevar a cabo el control, ciclo cerrado y ciclo abierto.
...
\item Control en lazo cerrado: aquí, el sistema embebido recibe información en tiempo real de sensores y ajusta su comportamiento en función de la retroalimentación. Esto permite corregir errores y mejorar la precisión del control. Un ejemplo clásico es el control de velocidad de un motor, donde sensores miden la velocidad real y ajustan ciertas variables para mantenerla en el valor deseado.
\end{itemize}

Para implementar un control por lazo cerrado se pueden aplicar la noción de ciclos de control. Se ejecutan ciertas operaciones de manera repetida a fin de lograr que una cierta variable llegue al valor deseado. Las operaciones que se llevan a cabo en el ciclo son las siguientes:
...
\item Cálculo: se compara los valores medidos con los de referencia a fin de determinar las acciones de control necesarias. Para hacerlo se utilizan algoritmos de control como, por ejemplo, algoritmos \gls{PID}.
