...
\begin{figure}[h]
\label{figDiseño}
\caption{Referencia gráfico de módulos}
\begin{center}
\begin{tikzpicture}
...
\node[módulo, below left=1cm and 4.3cm of NombreModulo, fill=yellow!20](mod){nombre};
...
\draw[tikzuml inherit style](x2a)--(x1a);
\node[below right=-0.4cm and 0.3cm of x2a, text width=4cm](hand){herencia};

\node[below=0.7cm of x1](x1b){};
\node[below=0.7cm of x2](x2b){};
\draw[tikzuml unidirectional aggregation style](x1b)--(x2b);
\node[right=0.3cm of x2b](rf){composición};
%
