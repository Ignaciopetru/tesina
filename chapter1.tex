...
Notemos que el DRV8838 tiene 3 pines de control, \textit{ENABLE}. \textit{PHASE} y \textit{SLEEP} pero solo utilizaremos los 2 primeros para simplificar el ejemplo. De todas formas, esta es la función de cada pin:
...
Por lo tanto, con los dos primeros pines podremos cumplir los requerimientos mencionados. Repasando, tendremos dos cables conectados a nuestro microcontrolador Arduino, uno que se dirige a \textit{ENABLE} y otro a \textit{PHASE}. Veamos ahora que código necesitamos escribir para poder hacer uso del motor:

\begin{lstlisting}[caption=Configuración inicial]
...
\begin{itemize}
    ...
    \begin{lstlisting}[caption=Configuración]
        void set_up() {
            .
            .
            .
...
Donde los parámetros del constructor \verb|MotorDC| son los pines de control, el parámetro de \verb|set_dir| es de tipo \verb|Dir| el cual es un enum que tiene dos entradas, horario y anti-horario y por ultimo el input de \verb|set_vel| es un int entre 0 y 255 que representa la velocidad porcentual. Veamos una posible implementación de esta interfaz utilizando el controlador DVR8838:

\begin{lstlisting}[caption=Constructor]
...
Además si debemos modificar el sentido de giro es tan fácil como cambiar la función \verb|set_dir| y listo, los clientes no notaran el cambio. A su vez, si queremos controlar otro motor podemos hacer lo siguiente:
...
Y para un cambio de componente, como el explicado anteriormente, los clientes del módulo lo ignoraran, ya que sera prácticamente la misma (a menos un cambio en el constructor, donde ya no se necesitaran los pines). Por ejemplo, ahora \verb|set\_dir| será:
...
\begin{figure}
\caption{Módulo MotorDC abstractor y estructura de herencia.}
\end{figure}
\begin{center}
\begin{tikzpicture}
...
Una posible implementación para las funciones \verb|escribir(i: str): bool| y \verb|limpiar()| del módulo \textbf{ControlEmca} es la siguiente:
...
\begin{center}
\begin{tikzpicture}
\umlsimpleclass[y=1]{Control}
\umlsimpleclass[y=-1]{Actuadores}

\umlsimpleclass[y=-3,x=-1]{Sensor1}
\umlsimpleclass[y=-3,x=1]{Sensor2}

\umltrans[geometry=-|-, anchor1=west,anchor2=west,arm1=-1cm]{Sensor1}{Control}
\umltrans[geometry=-|-,anchor1=east,anchor2=east,arm1=1cm]{Sensor2}{Control}
\umltrans[anchor1=-60,anchor2=60]{Control}{Actuadores}
\umltrans[anchor1=-120,anchor2=120]{Control}{Actuadores}
\umldep[anchor1=-159]{Actuadores}{Sensor1}
\umldep[anchor1=-21,]{Actuadores}{Sensor2}
...
\umlclass[x=2, y=1]{Sensor}
{}
{setConnection(i: Pipe) \\
signal()
}
\umlnote[x=3, y=4]{Sensor}{
signal()\{ \\
    value = getValue()\\
    pipe.write(value)\\
\}
...
\umlclass[x=7, y=-3]{Algoritmo}
{}
{calcular(): Measure
}
\umlclass[x=7, y=-6]{Data}
{}
...
\umluniaggreg{Control}{Data}

\end{tikzpicture}
\end{center}
\end{figure}

La manera en la que un \textit{cliente} utiliza el subsistema para lograr que la propiedad que controla vaya al valor que se desea es así:
...
El cliente ejecuta control() y la interrupción que marca el ciclo de trabajo ejecuta cicle(). De esta manera, cuando el estado es \textit{waiting}, cicle no hace nada y cuando esta en \textit{working} control no hace nada y la otra función se encarga de realizar el ciclo de control.
...
\umlnote[x=-2, y=4,width=5cm]{Timer}{
tickHandler() \{ \\
subsistemControlTimeout.execute() \\
\}
}

\umlsimpleclass[x=-5,y=-2]{Command}
...
\umlsimpleclass[x=3]{Data}
\umlsimpleclass[x=3, y=-1]{Algoritmo}
\umlsimpleclass[x=3, y=-2]{OperationState}
\umlsimpleclass[x=3, y=-3]{Pipe}

\umluniaggreg[anchor1=30,anchor2=west]{Controller}{Data}
\umluniaggreg[anchor1=17,anchor2=west]{Controller}{Algoritmo}
\umluniaggreg[anchor1=-1,anchor2=west]{Controller}{OperationState}
\umluniaggreg[anchor1=-18,anchor2=west]{Controller}{Pipe}
...
\umlclass[x=-3,y=-4]{OrdenExtensor}
{
...
\umlclass[x=5,y=-4]{OrdenMultiple}
{
...
\umlinherit[geometry=|-|]{OrdenMultiple}{Orden}
\umluniaggreg[geometry=-|-, anchor1=east, arm1=1cm]{OrdenMultiple}{Orden}
\umlnote[x=6,y=3,width=4cm]{OrdenMultiple}{
void ejecutar() \{  \\
    ordenRotor.ejecutar() \\
...
\begin{center}
\begin{tikzpicture}
\umlclass[x=-4,y=0]{MainController}
{
...
\umlclass[x=-8,y=-8]{MainWaiting}
{}
{
graspAt(i: Coordenadas) \\
notifyReady(i: MainCtrl) \\
}
\umlclass[x=-4,y=-8]{Main0}
{}
{
graspAt(i: Coordenadas) \\
notifyReady(i: MainCtrl) \\
...
\umlnote[x=-8,y=-4,width=5cm]{MainController}
{
void notifyReady() \{ \\
...
\node[módulo, below =4cm of CR](Pin){Sensor};
...
\umlnote[x=3,y=-10,width=5  cm]{Buffer}
{
signal() \{ \\
\ \ vColl.initPeriod() \\
\ \ t = vColl.periodTime()\\
...
\umlnote[x=-7,y=-5,width=4cm]{Collector}
{
addOne() \{ \\
\ \ nbr++ \}\\
\\
...
Registraremos un \textit{handler} para esta que será un módulo orden del patrón  \textit{Command} el cual encapsula que funiones son necesarias ejecutar para llevar a cabo la funcionalidad deseada. Por ejemplo, pedir la información de los sensores, ya sea a sus respectivos \textbf{Buffers} (si tenemos sensores como en el ejemplo anterior) o a los sensores en si, si son de otro tipo y luego registrarka en cierta módulo (que oculte una estructura de daros). De esta forma liberamos el procesador en los tiempos de espera y desacoplamos esta funcionalidad.



\section{Control anti-rebote}
Muchos dispositivos de entrada para sistemás embebidos utilizan contacto metal con metal para indicar eventos de interés, como botones, interruptores y relés. A medida que el metal entra en contacto, se produce una deformación física que resulta en un contacto intermitente de las superficies. Esto genera señales que de no ser filtradas pueden causar una lectura errónea. 
...
Por lo tanto el módulo PuertaAbierta implementará la función \verb|puertaAbierta|, completando en ella el comportamiento definido para la transición PuertaAbierta $\xrightarrow{}$ Calentando.
