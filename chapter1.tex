...
\begin{tabular}{|l|l|l|}
    ...
    \hline % Línea debajo del encabezado
...
Antes de abordar una perspectiva de diseño para gestionar el cambio, se analizará en detalle el ejemplo presentado en el libro de Douglass \cite{douglass} y la aplicación de otra solución al manejo de estados.
...
\based{Estado (State)}
\why{\textbf{Cambios previstos}: Pueden mofificarse los estados de control del botón en base a nuevos requerimientos o cambios en el hardware.
...
\declareCmod{Manejador}
\declareCmod{Recibida}
\declareCmod{NoRecibida}
...
\Manejador mantiene un estado interno que puede adoptar dos valores: \textit{Recibida}, si se ha recibido una interrupción del tipo correspondiente y está pendiente de ejecución, o \textit{NoRecibida}, si aún no se ha registrado ninguna. El comportamiento del módulo varía según el estado en que se encuentre.
...
\begin{figure}[H]
\caption{Ejemplo de manejador diferido de interrupciones usando \textit{State}.}
...
}
{
    ...
    \umlvirt{ejectuarInt(i: Manejador)} 
}
...
}
{
    ...
    ejectuarInt(i: Manejador) \\
    cambiarEstado(i: Estado)
}

\umlclass[below right=2cm and -2cm of Estado]{Recibida}
{
Recibida(i: NoRecibida)
}
{
    manejarInt(i: Manejador) \\
    ejectuarInt(i: Manejador)
...
\umlclass[below left=2cm and -2cm of Estado]{NoRecibida}
{
NoRecibida(i: Recibida)
}
{
    manejarInt(i: Manejador) \\
    ejectuarInt(i: Manejador)
...
\umlnote[below left=1cm and -1.5cm of Recibida,width=6cm]{Recibida}{
manejarInt(Manejador m) \{\}\\
ejectuarInt(Manejador m) \{ \\
\ \ \ \ m.cambiarEstado(noRecibida)\\
\ \ \ \ ...ejecutar int...\\
\}
}

\umlnote[left=1.5cm of NoRecibida,width=6cm]{NoRecibida}{
manejarInt(Manejador m) \{ \\
\ \ \ \ m.cambiarEstado(recibida)\\
\}
\\
ejectuarInt(Manejador m) \{\}
}
...
\umlinherit[geometry=|-|]{Recibida}{Estado}
\umlinherit[geometry=|-|]{NoRecibida}{Estado}
...
\label{docStateInt}
\scalebox{.85}{
\begin{pattern}[]{Control de manejo diferido de interrupciones}{Algorithm}{idFigAlg}
\based{State (Estado)}
\why{\textbf{Cambios previstos}: Agregar, quitar o editar estados de manejo de la interrupción.

\textbf{Funcionalidad}: Configura el estado actual de manejo de una interrupción a fin de que una interrupción se pueda ejecutar de manera diferida.
}
\assigns
\is{Manejador}{Contexto}
\is{Estado}{Estado}
\is{Recibida}{EstadoConcreto}
\is{NoRecibida}{EstadoConcreto}
...
\item \Pipe: Módulo que se ubica entre \Control y \Sensor. Tiene un funcionamiento sencillo, ofrece dos métodos: uno para escribir y otro para leer. De esta manera, un módulo escribe y otro lee, lo que permite comunicación entre ellos sin necesidad de invocación directa. Encapsula la comunicación entre módulos desacoplándolos.
\end{itemize}
...
\textbf{Funcionalidad}: Dependiendo del estado, los métodos control y cicle deben comportarse de manera diferente. A su vez, pueden cambiar de manera dinámica. En caso de que no se esté realizando una acción sobre alguno de los actuadores el estado será Waiting, en caso contrario Working.
}
\assigns
...
\declareCMod{Rotor}
\declareCMod{CmdRotorLeft}
\declareCMod{CmdRotorRight}
\declareCMod{SensorPosRotor}
...
\umlvirt{notifyEnd(i: MainCtrl)} \\
}
\umlclass[x=-10,y=-8]{MainWaiting}
{}
{
moveTo(i: Coordenadas) \\
notifyEnd(i: MainCtrl) \\
}
