...
En el diseño modular, un módulo consta de una parte visible llamada \textit{interfaz} y una oculta la llamada \textit{implementación}. La interfaz define el conjunto de métodos accesibles externamente, mientras que la implementación gestiona cómo el módulo lleva a cabo los comportamientos de cada método definido en la interfaz. Se dice que la implementación es invisible para el resto de módulos de sistema.
...
\umlnote[below=1cm of MotorGirarIzq]{MotorIniciarGiroIzq}{
ejecutar() \{ \\
\ \ \ \ motor.enable() \\
\ \ \ \ motor.left() \\
\}
...
\umlclass[above=1.3cm of MotorIniciarGiroIzq,type=abstract]{Orden}
{}
{
\umlvirt{ejecutar()}
}
...
\umlinherit{MotorIniciarGiroIzq}{Orden}
\umluniassoc[geometry=-|-]{MotorIniciarGiroIzq}{Motor}
...
\end{figure}
Un módulo \Controller, necesita manejar un \gls{pasoapaso} representado en el diseño por el módulo \Motor. Para hacerlo se deben ejecutar una serie de métodos de la interfaz del \Motor. En particular, primero se debe habilitar el giro del motor llamando al método \verb|enable| y luego configurar el sentido de giro con el método \verb|left| o \verb|right| (izquierda o derecha). Tradicionalmente, esto sería realizado desde el módulo \Controller agregando el código en el método \verb|control()| de este (ver ejemplo en Código \ref{notCommand}), provocando así, un fuerte acoplamiento entre \Controller y \Motor. Esto es, ante cualquier cambio de la interfaz de \Motor, se debe actualizar la implementación del módulo \Controller, es decir, hay que modificar dos módulos y como dijimos el sistema debe estar cerrado a cambios y abierto a extensiones. Reduciendo al mínimo la modificación de código existente lo que podría traer como resultado la introducción de errores.
...
\end{lstlisting}
Para aplicar el patrón \textit{Command} se debe crear el módulo heredero de \Orden que representa el comando en cuestión, en este caso, \MotorIniciarGiroIzq. Este encapsula cómo deben invocarse los métodos del módulo \Motor para que este realice un paso de giro hacia la izquierda. Por lo tanto, \Controller invocará el método ejecutar de \MotorIniciarGiroIzq para realizar dicha acción.

Los problemas del diseño tradicional se solucionan al aplicar este patrón. Se logra desacoplar al \Motor del \Controller: los posibles cambios en el módulo \Motor afectan solo al módulo \MotorIniciarGiroIzq. El \Controller desconoce cómo se lleva a cabo la acción de girar el motor un paso hacia la izquierda.
