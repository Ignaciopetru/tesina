...
Como se puede observar el estilo se basa en tres componenes básicos \textbf{Control}, \textbf{Proceso} y \textbf{Sensores}. Cada uno de ellos trabaja de manera independiente y se comunican mediante dos mecanismos principales, llamadas a prodedimientos y tubos (\textit{pipes}). Veamos qué información oculta cada uno:
...
\based{Orden (Command)}
\why{\textbf{Cambios previstos}: Se pueden agregar o modificar ordenes.

\textbf{Funcionalidad}: Desacoplar al que invoca una orden de que la lleva a cabo, proporcionando flexibilidad ante cambios en las ordenes o el receptor.
}
\assigns
\is{Orden}{Orden}
